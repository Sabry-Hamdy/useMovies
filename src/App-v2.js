    ...
    <>
      ...
      <Main>
        <Box>
          {isLoading && <Loader />}
          {!isLoading && !error && (
            <MoviesList movies={movies} onselectedMovieId={handleSelectedMovie} />
          )}
...
function MoviesList({ movies, onselectedMovieId }) {
  return (
    <ul className="list list-movies">
      {movies?.map((movie) => (
        <Movie movie={movie} key={movie.imdbID} onselectedMovieId={onselectedMovieId} />
      ))}
    </ul>
  );
}
...
function MovieDetails({ selectedMovieId, onCloseMovie, onAddWatched, watched }) {
  ...
  const movieUserRating = watched.find((movie) => movie.imdbID === selectedMovieId)?.userRating;
        ...
        <>
          ...
          <section>
            <div className="rating">
              {isWatched ? (
                <p>
                  You rated with movie {movieUserRating} <span>⭐️</span>
                </p>
              ) : (
                <>
